...
async def addlevel(interaction: discord.Interaction, position: int, name: str, gd_id: str, creator: str='', points: int=0):
    ...
    # Shift existing levels at or below the target down safely.
    # Use temporary negative positions first so UNIQUE(position) cannot collide.
    c.execute('DELETE FROM levels WHERE position=150')
    c.execute(
        "UPDATE levels SET position=-(position+1) "
        "WHERE position>=? AND position<150",
        (position,)
    )
    c.execute(
        "UPDATE levels SET position=-position "
        "WHERE position<0"
    )
